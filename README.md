...
Raw data values can vary wildly (e.g., NDVI might be small, while temperature is large). If we don't scale, the algorithm might bias itself toward _ Open `Symbolic_regression_DO_final.ipynb` or `RF_SVR_for_DO.ipynb`.
_ Run all cells.
...
- **Symbolic Regression**:
  ...
  - **Best Model**: Best equation with $R^2$ and MAE.
  - **Predictions**: `predicted_DO.csv` (Observed vs. Predicted).
- **RF & SVR**:
  - **Hyperparameters**: Saved to `best_rf_hyperparameters.csv` and `best_svr_hyperparameters.csv`.
  - **Predictions**: `rf_test_predictions.csv` and `svr_test_predictions.csv` (Inputs are scaled).
